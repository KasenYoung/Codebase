...
def _get_validation_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of visuals to save during visualization.",
    )
    parser.add_argument(
        "--grid_size",
        type=int,
...
def _get_training_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        default=False,
        help="Whether to use the regression loss on generator.",
    )
    parser.add_argument(
        "--num_denoising_step",
        type=int,
        default=2,
        help="Number of denoising steps to use for generator.",
    )
    parser.add_argument(
        "--backward_simulation",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Whether to use the backward simulation to sample denoised video latent.",
    )
    parser.add_argument(
        "--cls_on_video_latent",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Whether to use the gan loss.",
    )
    parser.add_argument(
        "--gan_alone",
        action="store_true",
        default=False,
        help="Whether to use the gan loss only, without dmd loss.",
    )
    parser.add_argument(
        "--dm_loss_weight",
        type=float,
        default=1.0,
        help="Weight of the dmd loss.",
    )
    parser.add_argument(
        "--gen_cls_loss",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Whether to use the gan loss on generator turn.",
    )
    parser.add_argument(
        "--gen_cls_loss_weight",
        type=float,
        default=1.0,
        help="Weight of the gan loss in the generator turn.",
    )
    parser.add_argument(
        "--guidance_cls_loss_weight",
        type=float,
        default=1.0,
        help="Weight of the gan loss in the guidance turn.",
    )
    parser.add_argument(
        "--min_step_percent",
        type=float,
        default=0.0,
        help="Minimum step percentage to be sampled for the diffusion steps.",
    )
    parser.add_argument(
        "--max_step_percent",
        type=float,
        default=1.0,
        help="Maximum step percentage to be sampled for the diffusion steps.",
    )
    parser.add_argument(
        "--real_guidance_scale",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="The ratio of generator updates and discriminator updates.",
    )
    parser.add_argument(
        "--load_from_regression",
        action="store_true",
        default=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--mix_spatial_temporal",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Whether to mix spatial and temporal training."
    )
    parser.add_argument(
        "--mix_iter_freq",
        type=int,
        default=1,
        help="The frequency of mixing spatial and temporal training."
    )
...
def _get_configuration_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Number of iterations between wandb logging.",
    )
    parser.add_argument(
        "--rocm",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Whether or not to use ROCm.",
    )


def _get_regression_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--regression_video_file",type= str, help="path to video names.")
    parser.add_argument("--regression_data_path",type=str, help="path to data.")
    parser.add_argument("--regression_timesteps",nargs='+', type=int, help='List of timesteps')
    parser.add_argument("--regression_steps",type=int,help="the regression steps.")
    parser.add_argument("--save_regression",type= int, help="the interval of save models.")


def _get_cm_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--use_ema",
        action=argparse.BooleanOptionalAction,
        default=False,
    ...
    parser.add_argument(
        ...
        type = int,
        default = 50,
        help="The number of steps in CM distillation.",
    )
    parser.add_argument(
        "--cm_cfg",
        type = float,
        default = 0.0,
        help = "CFG scale in CM. When set 0, it means no CFG."
    )
    parser.add_argument(
        "--cm_loss_type",
        type = str,
